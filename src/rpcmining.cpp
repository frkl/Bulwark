...
void FormatHashBuffers(CBlock* pblock, char* pmidstate, char* pdata, char* phash1)
{
  ...
  tmp.block.hashMerkleRoot = pblock->GetBlockHeader().hashMerkleRoot;
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        // Save
        mapNewBlock[pblock->GetBlockHeader().hashMerkleRoot] = make_pair(pblock, pblock->vtx[0].vin[0].scriptSig);
    ...
    else
    {
        ...
        // Get saved block
        if (!mapNewBlock.count(pdata->GetBlockHeader().hashMerkleRoot))
            return false;
        CBlock* pblock = mapNewBlock[pdata->GetBlockHeader().hashMerkleRoot].first;
